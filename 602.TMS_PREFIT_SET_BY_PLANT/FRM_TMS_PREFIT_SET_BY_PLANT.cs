...
namespace FORM
{
    public partial class FRM_TMS_PREFIT_SET_BY_PLANT : Form
    {
        public FRM_TMS_PREFIT_SET_BY_PLANT()
        {
            InitializeComponent();
        }
        #region Variable
        ...
        #region Ora
        private DataTable TMS_HOME_SET_BY_PLANT_SELECT(string ARG_TYPE, string ARG_FACTORY,string ARG_PLANT, string ARG_PROC_CD, string ARG_IS_BOTTOM)
        {
            try
            {
                ...
                string process_name = "MES.PKG_TMS_PREFIT.TMS_HOME_SET_PLANT_SELECT";
        ...
        private void BindingData(string Qtype)
        {
            ...
            try
            {
                ...
                string PlantCD = ComVar.Var._Area.Split('_')[0];
                DataTable dt1 = TMS_HOME_SET_BY_PLANT_SELECT(Qtype, ComVar.Var._strValue1, PlantCD, ComVar.Var._strValue2, ComVar.Var._bValue1?"1":"0");
        ...
        private void FRM_TMS_PREFIT_SET_BY_PLANT_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                try
                {
                    ...
                    string PlantNM = ComVar.Var._Area.Split('_')[1];
                    //lblTitle.Text = string.Concat(System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(ComVar.Var._strValue3.ToLower()), " Set Balance At " + PlantNM);
                    lblTitle.Text = string.Concat(System.Globalization.CultureInfo.CurrentCulture.TextInfo.ToTitleCase(ComVar.Var._strValue3.ToLower()), " Set Balance At Supermarket");
