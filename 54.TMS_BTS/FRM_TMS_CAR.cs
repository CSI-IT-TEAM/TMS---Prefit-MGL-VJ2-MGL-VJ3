      ...
      //  Dictionary<string,string> _dtnTrip = new Dictionary<string,string>();
        ...
        private System.Data.DataSet Select_Ora_Grid()
        {
            try
            {
                ...
                string process_name = "MES.PKG_TMS.TMS_MAT_SEL_MAT";
                //ARGMODE
        ...
        private void load_Data()
        {
            try
            {
                _dtData = Select_Ora_Grid().Tables[0];
                _dtn_Trip.Clear();
                for (int i = 0; i < _dtData.Rows.Count; i++)
                {
                    if (!_dtn_Trip.ContainsKey(_dtData.Rows[i]["TRIP"].ToString()))
                    {
                        _dtn_Trip.Add(_dtData.Rows[i]["TRIP"].ToString()
                                        , new Tuple<string, string, string>(_dtData.Rows[i]["LEAVE_LAMI"].ToString()
                                            , _dtData.Rows[i]["DEPART_TIME"].ToString()
                                            , _dtData.Rows[i]["ARIVAL_TIME"].ToString()));
                    }
                }
