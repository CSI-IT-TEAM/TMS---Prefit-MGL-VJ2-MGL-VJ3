...
namespace FORM
{
    public partial class FRM_MGL_BTS_MONTHLY_S : Form
    {
        public FRM_MGL_BTS_MONTHLY_S()
        {
            InitializeComponent();
        ...
        UC.UC_DWMY DWMY = new UC.UC_DWMY(3,"");
        ...
        private void Form_VisibleChanged(object sender, EventArgs e)
        {
            ...
            if (this.Visible)
            {
                lblTitle.Text = ComVar.Var._strValue1.Equals("TOTAL1") ? "VJ1 Support BTS by Month" : ComVar.Var._strValue2 + " BTS by Month";
                DWMY.YMD_Change(3,"");
                _iCountReload = 40;
                tmrDate.Start();
        ...
        private void formatband()
        {
            try
            {
                ...
                dtsource = SMT_EMD_BTS_MONTHLY_SELECT("H", uc_year.GetValue().ToString(), ComVar.Var._strValue1, "");
                                ...
                                for (int i = 0; i < dtsource.Rows.Count; i++)
                                {
                                    if (band.Name.Contains(dtsource.Rows[i][0].ToString().Substring(dtsource.Rows[i][0].ToString().Length - 2)))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            band.AppearanceHeader.BackColor2 = Color.RoyalBlue;
                                            band.AppearanceHeader.ForeColor = Color.White;
                                        }

                                        band.Caption = dtsource.Rows[i]["CAPTION"].ToString().Replace("_", "\n").Replace("CURR", "");
                                        band.RowCount = 2;
                                        band.AppearanceHeader.Font = new Font("Calibri", 12, FontStyle.Bold);
                                        break;
                                    }
        ...
        //    try
        //    {
        //        DataTable dt = SMT_EMD_BTS_MONTHLY_SELECT("C", uc_year.GetValue().ToString(), ComVar.Var._strValue1, "");
        ...
        private void BindingDataGrid()
        {
            try
            {
                ...
                DataTable dt = SMT_EMD_BTS_MONTHLY_SELECT("Q", uc_year.GetValue().ToString(), ComVar.Var._strValue1, "");
