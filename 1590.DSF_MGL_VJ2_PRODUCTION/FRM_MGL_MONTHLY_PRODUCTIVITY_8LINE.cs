...
namespace FORM
{
    public partial class FRM_MGL_MONTHLY_PRODUCTIVITY_8LINE : Form
    {
        
    

        public FRM_MGL_MONTHLY_PRODUCTIVITY_8LINE()
        {
            InitializeComponent();
        }
       ...
       Form[] arrForm = new Form[3];
        ...
        public FRM_MGL_MONTHLY_PRODUCTIVITY_8LINE(string Title, int _indexScreen, string wh_cd, string mline_cd)
        {
           ...
           // lblGaugesValue.Text = "0";
            if (arg_dt != null && arg_dt.Rows.Count > 0)
            {
                try
                {
                    ...
                    arcScaleTrucks.MinValue = Convert.ToSingle(arg_dt.Rows[0]["MIN_VALUE_POD"]);
                    arcScaleTrucks.MaxValue = Convert.ToSingle(arg_dt.Rows[0]["MAX_VALUE_POD"]);
                    arcScaleTrucks.Ranges[0].StartValue = Convert.ToSingle(arg_dt.Rows[0]["MIN_VALUE_POD"]);
                    arcScaleTrucks.Ranges[0].EndValue = arcScaleTrucks.Ranges[1].StartValue = Convert.ToSingle(arg_dt.Rows[0]["YELLOW_VALUE_POD"]); ;
                    arcScaleTrucks.Ranges[1].EndValue = arcScaleTrucks.Ranges[2].StartValue = Convert.ToSingle(arg_dt.Rows[0]["GREEN_VALUE_POD"]); ;
                    arcScaleTrucks.Ranges[2].EndValue = Convert.ToSingle(arg_dt.Rows[0]["MAX_VALUE_POD"]); 
                    ...
                    lblRed.Text = "<" + arg_dt.Rows[0]["YELLOW_VALUE_POD"].ToString();
                    lblYellow.Text = arg_dt.Rows[0]["YELLOW_VALUE_POD"].ToString() + " ~ " + arg_dt.Rows[0]["GREEN_VALUE_POD"].ToString();
                    lblGreen.Text = ">" + arg_dt.Rows[0]["GREEN_VALUE_POD"].ToString();
            ...
            try
            {
                ...
                for (int i = 0; i < arg_dt.Rows.Count; i++)
                {
                    if (arg_dt.Rows[i]["POD"] == null || arg_dt.Rows[i]["POD"].ToString() == "")
                        ...
                        series2.Points.Add(new SeriesPoint(arg_dt.Rows[i]["DD"].ToString() + "\n" + arg_dt.Rows[i]["DY"].ToString(), arg_dt.Rows[i]["POD"]));
                }
        ...
        private void load_data_grid(DataTable arg_dt)
        {
            try
            {
                if (arg_dt != null && arg_dt.Rows.Count > 0)
                {
                    ...
                    axGrid.SetText(2, 1, arg_dt.Rows[0]["MON"].ToString());
                    ...
                    for (int i = 0; i < arg_dt.Rows.Count; i++)
                    {
                        ...
                        axGrid.SetText(icol, 9 * (iNum - 1) + 3, arg_dt.Rows[i]["ACT_QTY"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 4, arg_dt.Rows[i]["TOT_MAN"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 5, arg_dt.Rows[i]["TOT_ATTN"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 6, arg_dt.Rows[i]["TOT_DIRECT"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 7, arg_dt.Rows[i]["TOT_INDIRECT"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 8, arg_dt.Rows[i]["TOT_RELIEF"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 9, arg_dt.Rows[i]["TOT_ABSENT"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 10, arg_dt.Rows[i]["POH"].ToString());
                        axGrid.SetText(icol, 9 * (iNum - 1) + 11, arg_dt.Rows[i]["POD"].ToString());
                        for (int j = 9 * (iNum - 1) + 3; j <= 9 * (iNum - 1) + 11; j++)
                        {
                            axGrid.SetText(1, j, arg_dt.Rows[i]["MLINE_NM"].ToString());
                        }
        ...
        private void Load_Row_Head(DataTable arg_dt)
        {
            ...
            {
                axGrid.SetText(2, 9 * i + 3, "Production");
                axGrid.SetText(2, 9 * i + 4, "Man-Power");
                axGrid.SetText(2, 9 * i + 5, "Total Attendant");
                axGrid.SetText(2, 9 * i + 6, "    Direct");
                axGrid.SetText(2, 9 * i + 7, "    Indirect");
                axGrid.SetText(2, 9 * i + 8, "    IE Relieft");
                axGrid.SetText(2, 9 * i + 9, "Asbent");
                axGrid.SetText(2, 9 * i + 10, "POH");
                axGrid.SetText(2, 9 * i + 11, "POD");
                axGrid.Col = -1;
                axGrid.Row = 9 * i + 11;
                axGrid.BackColor = Color.Orange;
                axGrid.ForeColor = Color.White;
        ...
        {
            string[] str_col = { "DD", "DY", "TARGET", "POD" };
            // return arg_dt.Select(arg_expression, arg_sortOrder).CopyToDataTable().DefaultView.ToTable(true, arg_column);
            return arg_dt.Select("MLINE_CD = '" + arg_expression + "' and THEDATE <>'99999999'", "THEDATE").CopyToDataTable().DefaultView.ToTable(true, str_col);
        ...
        private void load_data()
        {
            ...
            if (_wh_cd == "VJ2")
            {
                ...
                CreateChartLine(Chart6, select_Data("202", ds.Tables[0]), "Lean LE");
            ...
            else
            {
                ...
                CreateChartLine(Chart3, select_Data("002", ds.Tables[0]), "Line 3");
                CreateChartLine(Chart4, select_Data("003", ds.Tables[0]), "Line 4");
                CreateChartLine(Chart5, select_Data("004", ds.Tables[0]), "Line 5");
                CreateChartLine(Chart6, select_Data("005", ds.Tables[0]), "Line 6");
                tblMain.RowStyles[0].SizeType = SizeType.Percent;
                tblMain.RowStyles[0].Height = 50;
            }

            BindingPOD(ds.Tables[0].Select("thedate = '99999999' and mline_cd = '999'").CopyToDataTable());
        ...
        public System.Data.DataSet LOAD_DATA_v2()
        {
            ...
            try
            {
                string process_name = "MES.PKG_MGL_VJ2.MGL_PRODUCTIVITY_MONTH";
                ...
                MyOraDB.Parameter_Values[0] = "";     
                MyOraDB.Parameter_Values[1] = _wh_cd;
                MyOraDB.Parameter_Values[2] = _mline_cd;
        ...
        private void FORM_IPEX3_LOGISTIC_VisibleChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.Visible)
                {
                    ...
                    lblTitle.Text = ComVar.Var._strValue1.Equals("TOTAL2") ? "VJ2 Productivity Status by Month" : ComVar.Var._strValue2 + " Productivity Status by Month";
