...
namespace FORM
{
    public partial class FRM_MGL_WEEKLY_PRODUCTION : Form
    {
        ...
        public DataTable SMT_PROD_WEEKLY_SELECT(string ARG_DATE, string ARG_LINE_CD, string ARG_MLINE_CD)
        {
            ...
            try
            {
                string process_name = "MES.PKG_MGL_VJ3.MGL_PRODUCTION_WEEKLY_SELECT";
        ...
        private void BindingChart()
        {
            ...
            dt = SMT_PROD_WEEKLY_SELECT(DateTime.Now.ToString("yyyyMMdd"), ComVar.Var._strValue1, mline);
            //InitChart("UPS1", dt, chartUPS1); InitChart("UPS2", dt, chartUPS2); InitChart("UPS3", dt, chartUPS3);
            InitDataChart("UPC", dt, chartUPC);
            InitDataChart("UPS1", dt, chartUPS1); 
            InitDataChart("UPS2", dt, chartUPS2);
            InitChart("UPS3", dt, chartUPS3);
        }
        ...
        private void FRM_PROD_STATUS_DAILY_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                lbltitle.Text = ComVar.Var._strValue1.Equals("TOT") ? "VJ1,VJ2 Support Production Status by Week" : ComVar.Var._strValue1 + " Support Production Status by Week";
