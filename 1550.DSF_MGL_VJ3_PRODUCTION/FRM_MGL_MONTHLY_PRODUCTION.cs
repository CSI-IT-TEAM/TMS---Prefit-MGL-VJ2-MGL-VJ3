...
namespace FORM
{
    public partial class FRM_MGL_MONTHLY_PRODUCTION : Form
    {
        ...
        public DataTable SMT_PROD_MONTHLY_SELECT(string ARG_TYPE, string ARG_DATE, string ARG_LINE_CD, string ARG_MLINE_CD)
        {
            ...
            try
            {
                string process_name = "MES.PKG_MGL_VJ3.MGL_PRODUCTION_MONTHLY_SELECT";
        ...
        private void FRM_PROD_STATUS_DAILY_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                lbltitle.Text = ComVar.Var._strValue1.Equals("TOT") ? "VJ1,VJ2 Support Production Status by Month" : ComVar.Var._strValue1 + " Support Production Status by Month";
