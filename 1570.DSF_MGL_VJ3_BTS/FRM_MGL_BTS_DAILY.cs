...
namespace FORM
{
    public partial class FRM_MGL_BTS_DAILY : Form
    {
        public FRM_MGL_BTS_DAILY()
        {
            InitializeComponent();
        ...
        private void Form_VisibleChanged(object sender, EventArgs e)
        {
            if (this.Visible)
            {
                lblTitle.Text = ComVar.Var._strValue1.Equals("TOT") ? "VJ1,VJ2 Support BTS by Day" : ComVar.Var._strValue1 + " Support BTS by Day";
                DWMY.YMD_Change(1); DWMY.YMD_Change(2);
                _iCountReload = 60;
                tmrDate.Start();
            }
            else
        ...
        private void tmrDate_Tick(object sender, EventArgs e)
        {
            ...
            catch { _iCountReload = 0; this.Cursor = Cursors.Default; }
        ...
        public DataTable SMT_EMD_BTS_MONTHLY_SELECT(string ARG_TYPE, string ARG_DATE, string ARG_LINE_CD, string ARG_MLINE_CD)
        {
            ...
            try
            {
                string process_name = "MES.PKG_MGL_VJ3.MGL_BTS_DAY_DATA_SELECT";
                            ...
                            if (double.TryParse(band.Name.Substring(4, 2), out num))
                            {
                                ...
                                for (int i = 0; i < dtsource.Rows.Count; i++)
                                {
                                    if (band.Name.Contains(dtsource.Rows[i][0].ToString().Substring(dtsource.Rows[i][0].ToString().Length - 2)))
                                    {
                                        ...
                                        band.RowCount = 2;
                                        band.AppearanceHeader.Font = new Font("Calibri", 16, FontStyle.Bold);
                                        break;
                                    }
